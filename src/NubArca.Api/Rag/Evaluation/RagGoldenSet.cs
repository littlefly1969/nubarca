...
public sealed record RagGoldenCase(
    ...
    /// Whether this query is a NATURAL-LANGUAGE prompt that must not appear
    /// verbatim anywhere in the corpus it is measured against.
    ///
    /// Not every golden query wants this. `PhotoVectorIndexService` and
    /// `RevisionMismatch_DoesNotCallModel` are IDENTIFIERS, and the whole point
    /// of those cases is that the literal string occurs in the file that should
    /// win — guarding them would forbid the behaviour being tested. A question
    /// somebody types in prose is the opposite: if it occurs in the corpus, the
    /// benchmark can no longer tell "found the implementation" from "found a
    /// document repeating the question".
    ///
    /// The set is chosen deliberately, case by case. It is not inferred from
    /// string length or from whether the text contains a space.
    bool Conceptual = false)
{
    ...
    /// A prose question. Marks the case as contamination-guarded.
    public static RagGoldenCase Prose(
        string domain, string language, string query, params string[] expected)
        => new(domain, language, query, expected, Array.Empty<string>(), Conceptual: true);
}
...
public static class RagGoldenSet
{
    ...
    public static IReadOnlyList<RagGoldenCase> ProductHelp { get; } = new[]
    {
        ...
        new RagGoldenCase(
            ...
            new[] { "docs/help/faces" }, new[] { "docs/OPERATIONS.md" }, Conceptual: true),
        RagGoldenCase.Prose(
            RagDomains.ProductHelp, RagLanguages.Italian,
