...
///
/// LOOK AT THE CONSTRUCTOR. A text-only model runtime, a product-help retriever,
/// the model resolver, a logger. There is no DbContext, no file service, no
...
/// artifact service, and no IServiceProvider to go looking for one. This class
/// could not reach a user's library if its prompt asked it to, because nothing
/// here can read one.
///
...
public sealed class HelpAssistantService
{
    ...
    public HelpAssistantStatus GetStatus()
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // different revision. Calling the model anyway would buy an answer
        // improvised with no product documentation behind it — the answer most
        // likely to be wrong about the version actually installed — and for an
        // External model it would also cross the privacy boundary to do it.
        //
        // Still an optional-feature failure, never an application-health one.
        if (!_knowledge.IsAvailable)
        {
            _log.LogInformation(
                "help: refused, no approved product knowledge for the running revision");
            return Failed(AssistantFailureReasons.KnowledgeUnavailable);
        }

        var retrieval = _knowledge.Retrieve(new RagQuery(
            RagDomainKey.ProductHelp,
            trimmed,
            bounds.EffectiveEvidenceChunks,
            bounds.EffectiveEvidenceCharacters));

        // Outcome and count, never the question and never the excerpt text.
        _log.LogInformation(
            "help: retrieval domain={Domain} outcome={Outcome} evidence={Count}",
            RagDomainKey.ProductHelp.Value, retrieval.Outcome, retrieval.Evidence.Count);
