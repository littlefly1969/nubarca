...
// browser shell navigates by. Pure, testable without a DOM.
//
// A PORT of tv/src/lib/assignmentView.ts, and deliberately not a redesign: a
// browser running /tv is the same NubArca display as a Fire TV, so it reads the
// control plane with the same words, the same cadence and the same fail-closed
// rules. The two copies cannot be one file — the native app ships as a TV
// release and has its own toolchain — so `nativeParity.test.ts` loads the
// native module and runs both against the same cases. Change one without the
// other and that test fails.
...
/**
 * How often a visible, paired television reads the control plane. It is the
 * bound on a takeover: an owner assigning a party, a game starting, finishing
 * or restarting all reach the screen within one interval. A READ — it writes
 * nothing — so it can afford to be brisk.
 */
export const CONTROL_POLL_MS = 5_000;

/**
 * How often one of those reads is the session HEARTBEAT instead: the write that
 * keeps `LastSeenAt` honest in the owner's device list, once a minute rather
 ...
 * answer — no network, a 5xx, a timeout. Exponential and capped, so a server
 * that is down for an hour is asked twice a minute rather than in a loop. Used
 * wherever the display must keep trying on its own: validating the session at
 * boot (a mini-PC that powers on before its network must not UNPAIR itself)
 ...
 * meant "that party is over". A slideshow without an album fails closed.
 */
