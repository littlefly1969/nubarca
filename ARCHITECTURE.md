...
**Visual document retrieval** adds a second way to FIND a document and changes nothing about what may be said from it. Pages are rendered locally, embedded with the SigLIP2 So400m towers the photo library already runs — under a document-specific profile and document-specific storage, so the two spaces can never be compared or reindexed as one another — and a hit narrows the ordinary private text retrieval to the documents it named. The two ranked lists are fused by rank, the existing evidence gate decides, and the final evidence is still bounded `DocumentChunk` text. A `DocumentVisualUnit` never becomes `RagEvidence`, no page image reaches a generative model, and a visually perfect page whose text says nothing relevant produces **no model call at all** — visual similarity is not permission to improvise.

Rendered pages are never stored: render, embed, discard, keeping only a pixel hash so a rebuild can prove determinism. Publication is all or nothing — a twenty-page contract whose page 13 failed contributes zero hits from pages 1–12, and the database refuses a `Completed` index with no units. Eligibility is recomputed live on every query and adds three conditions to the text ones: the index must be complete, its blob must be the file's CURRENT blob (which invalidates a replaced document instantly, with no sweeper), and both the render and embedding profiles must be the active ones.

Office documents are laid out by a container of their own. Open XML parsing reads words and says nothing about where they sit, and where they sit is the signal — but a layout engine over a file somebody uploaded does not belong in a process holding database credentials. The API sends bytes and a format ORDINAL over a Unix socket to a worker with no network route, no credentials and no owner identity, and receives a PDF; the protocol has no way to express a path, a filename, a command, an import filter or a URL. A rendered office page ordinal is that engine's pagination and is never a citation: Slice-4 typed text provenance stays the authority.

The pgvector accelerator for visual embeddings deliberately has **no ANN index**. An approximate index plus an owner predicate is not an owner-prefiltered search, so its absence is what leaves the planner one plan: restrict through the eligibility joins, then rank exactly. Late interaction is a bounded, model-agnostic seam with an exact MaxSim; no candidate is promoted in this release, and the dense path is a complete configuration on its own.
