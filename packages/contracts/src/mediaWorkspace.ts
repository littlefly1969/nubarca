// The media-workspace query model (§7-§9, §11, §17, §18, §19).
//
// One filter vocabulary for every client. This used to live inside the web
// app; it is pure — no React, no i18n, no side effects — and there was never
// anything web-specific about what a filter MEANS, only about how it is drawn.
//
// The workspace serves two sources (library, album) x three media kinds
// (all | image | video) x two library scopes (active | excluded) from ONE
// contract. Filters are grouped so an adaptive UI can show exactly the
// controls valid for the current kind, and so incompatible filters are NEVER
// sent silently to the backend:
//
//   Common  -> valid for every kind (and the only filters shown in "all")
//   Photo   -> only valid with kind=image
//   Video   -> only valid with kind=video
//
// URL persistence deliberately stayed in the web client: a phone has no
// address bar, and URLSearchParams has no place in a platform-neutral package.

import type { AlbumMembership, ListMediaQuery, MediaSortDirection, MediaSortField } from './media.ts';
